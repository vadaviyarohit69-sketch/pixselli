...
import { blogSpanishArticles, type BlogSlug } from '@/lib/blogSpanishArticles';
import { blogSpanishEnhancements } from '@/lib/blogSpanishEnhancements';

interface SpanishBlogPostPageProps {
  slug: BlogSlug;
}

export default function SpanishBlogPostPage({ slug }: SpanishBlogPostPageProps) {
  const post = getBlogPostBySlug(slug);
  const content = blogSpanishArticles[slug];
  const enhancement = blogSpanishEnhancements[slug];
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <BlogStructuredData slug={slug} locale="es" faqs={enhancement.faqs} />
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14 md:py-20">
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-bold leading-tight mb-4">{post.title.es}</h1>
            <p className="text-white/85 text-base md:text-lg mb-6">{post.heroSubtitle.es}</p>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10">
        <div className="relative h-56 sm:h-72 md:h-96 rounded-2xl overflow-hidden shadow-xl">
          <Image src={post.coverImage} alt={post.title.es} fill className="object-cover" priority />
        </div>
      </section>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="prose prose-lg max-w-none prose-headings:text-gray-900 prose-p:text-gray-700">
          ...
          <section className="mt-12 p-6 rounded-2xl border border-teal-100 bg-teal-50">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Consejos rapidos</h2>
          ...
          <section className="mt-12 p-6 rounded-2xl border border-rose-100 bg-rose-50">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Errores comunes</h2>
            <ul className="space-y-3">
          ...
          <section className="mt-12 p-6 rounded-2xl border border-indigo-100 bg-indigo-50">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Checklist final</h2>
            <ul className="space-y-3">
